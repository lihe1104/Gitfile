    ...
    public void progressChanged(ProgressEvent progressEvent) {
        ...
        switch (eventType) {
            ...
            case TRANSFER_STARTED_EVENT:
                if (videoId != null) {
                    LOGGER.debug ("Start to upload videoId " + videoId + "......");
                }
                if (imageId != null) {
                    LOGGER.debug("Start to upload imageId " + imageId + "......");
                }
                break;
            ...
            case REQUEST_BYTE_TRANSFER_EVENT:
                ...
                if (this.totalBytes != -1) {
                    ...
                    LOGGER.debug(bytes + " bytes have been written at this time, upload sub total : " +
                            "(" + this.bytesWritten + ")");
                }
                break;
            // 文件全部上传成功事件通知
            case TRANSFER_COMPLETED_EVENT:
                this.succeed = true;
                if (videoId != null) {
                    LOGGER.debug("Succeed to upload videoId " + videoId + " , " + this.bytesWritten + " bytes have been transferred in total.");
                }
                if (imageId != null) {
                    LOGGER.debug("Succeed to upload imageId " + imageId + " , " + this.bytesWritten + " bytes have been transferred in total.");
                }
                break;
            // 文件上传失败事件通知
            case TRANSFER_FAILED_EVENT:
                if (videoId != null) {
                    LOGGER.debug("Failed to upload videoId " + videoId + " , " + this.bytesWritten + " bytes have been transferred.");
                }
                if (imageId != null) {
                    LOGGER.debug("Failed to upload imageId " + imageId + " , " + this.bytesWritten + " bytes have been transferred.");
                }
                break;
